...
# sys.path.append("/home/omnisky/shenxiaochen/path_miccai")
import argparse
import torch
...
parser.add_argument('--data_folder', type=str, default='/home/hpcstack/data/shen_chen_path_mouse/data/', help='path to custom dataset')
...
parser.add_argument('--train_percent', type=float, default=0.01, help='train dataset percent')
...
parser.add_argument('--save_checkpoint', type=str, default="objcovstd-1", help='name of saved checkpoint')
...
def main():
    ...
    # 数据位置
    traindir = args.data_folder + "train_data"
    valdir = args.data_folder + "dev_data"
    testdir = args.data_folder + "test_data"
    ...
    if args.train_percent < 1.0:
        train_files = pd.read_csv("./train_image_dir.txt", sep="\t", header=None)
        train_samples = train_files.groupby(3, group_keys=False).apply(lambda x: x.sample(frac=args.train_percent,random_state=1412))
        train_dataset.samples = []
        for i in range(len(train_samples)):
            cls = train_samples.iloc[i,0].strip().split("/")[-2]
            train_dataset.samples.append((train_samples.iloc[i,0].strip(),train_dataset.class_to_idx[cls]))
    ...
    checkpoint_path = 'checkpoints/FPath-linate-mouse-percent-{}-{}.pth'.format(args.save_checkpoint,args.epochs)  # here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (images, target) in enumerate(
            ...
            if (step + 1) % args.print_freq == 0 and args.local_rank == 0:
                ...
                print('Train: [{0}][{1}/{2}]\t'
                    ...
                    epoch, ((step + 1) - (epoch * len(train_loader))), len(train_loader), loss=losses,
        ...
        if args.local_rank == 0:
            ...
            print('Epoch: [{0}]\t'
                  'val_ACC {top1_val.val:.3f} ({top1_val.avg:.3f})\t'.format(epoch, top1_val=top1_val))
